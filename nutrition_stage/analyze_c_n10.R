...
out_dir <- "nutrition_stage/output_c_n10"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
dat <- read.csv(file.path(out_dir, "c_n10_analysis_core.csv"), na.strings = c("", "NA"))

numeric_vars <- c(
  "age", "sex", "race", "education", "pir", "weight", "psu_u", "strata_u",
  "alpha_tocopherol_umol_l", "total_cholesterol_mg_dl", "total_cholesterol_mmol_l",
  "hdl_mg_dl", "vit_e_chol_ratio", "log_ratio", "log_alpha_tocopherol", "b12_pg_ml",
  "mma_umol_l", "log_mma", "folate_nmol_l", "folate_ng_ml", "hba1c",
  "creatinine_mg_dl", "egfr", "albumin_g_dl", "total_protein_g_dl", "bmi",
  "body_weight_kg", "height_cm", "diabetes", "left_insensate", "right_insensate",
...
dat$group <- factor(dat$group, levels = c("concordant_adequate", "ratio_low_absolute_normal", "absolute_low"))
...
model_rows <- list()
append_model <- function(scope, model_name, model, term = "groupratio_low_absolute_normal", exponentiate = FALSE) {
  z <- safe_effect(model, term, exponentiate)
  model_rows[[length(model_rows) + 1]] <<- data.frame(
    candidate_code = "C-N10", scope = scope, model = model_name, term = term,
...
full_rhs <- "group + age + sex + race + bmi + smoking + hba1c + egfr + b12_pg_ml + log_mma + folate_nmol_l + pir + albumin_g_dl + cycle"
fit_scope <- function(d, scope) {
  d <- d[d$complete_case & !is.na(d$group), , drop = FALSE]
  d$group <- droplevels(factor(d$group, levels = c("concordant_adequate", "ratio_low_absolute_normal", "absolute_low")))
  if (nrow(d) < 250 || sum(d$group == "ratio_low_absolute_normal") < 30 || sum(d$pn_any == 1) < 20) return(NULL)
  des <- make_design(d)
  crude_pr <- svyglm(pn_any ~ group + cycle, design = des, family = quasipoisson(link = "log"))
  crude_rd <- svyglm(pn_any ~ group + cycle, design = des, family = gaussian())
...
if (is.null(primary)) stop("Primary C-N10 model could not be estimated")
...
miss_z <- safe_effect(miss_model, "groupratio_low_absolute_normal", exponentiate = TRUE)
missingness_df <- data.frame(
  term = "groupratio_low_absolute_normal",
  odds_ratio = miss_z[1], lower = miss_z[2], upper = miss_z[3], p = miss_z[4],
  stringsAsFactors = FALSE
)

# Restricted cubic spline of lipid-adjusted vitamin E among absolute-normal participants.
...
nl <- primary$data[primary$data$alpha_tocopherol_umol_l >= 12, , drop = FALSE]
knots <- weighted_quantile(nl$log_ratio, nl$weight, c(0.05, 0.35, 0.65, 0.95))
basis <- rcs_basis(nl$log_ratio, knots)
nl$xlin <- nl$log_ratio
nl$rcs1 <- basis[, 1]
nl$rcs2 <- basis[, 2]
des_nl <- make_design(nl)
nl_pr <- svyglm(
  pn_any ~ xlin + rcs1 + rcs2 + age + sex + race + bmi + smoking + hba1c + egfr + b12_pg_ml + log_mma + folate_nmol_l + pir + albumin_g_dl + cycle,
  design = des_nl, family = quasipoisson(link = "log")
)
nl_strict <- svyglm(
  pn_two_plus ~ xlin + rcs1 + rcs2 + age + sex + race + bmi + smoking + hba1c + egfr + b12_pg_ml + log_mma + folate_nmol_l + pir + albumin_g_dl + cycle,
  design = des_nl, family = quasipoisson(link = "log")
)
test_pr <- regTermTest(nl_pr, ~rcs1 + rcs2)
test_strict <- regTermTest(nl_strict, ~rcs1 + rcs2)
nonlinear_df <- rbind(
  data.frame(outcome = "pn_any", n = nrow(nl), p_nonlinear = unname(test_pr$p), knots_ratio = paste(round(exp(knots), 3), collapse = "|")),
  data.frame(outcome = "pn_two_plus", n = nrow(nl), p_nonlinear = unname(test_strict$p), knots_ratio = paste(round(exp(knots), 3), collapse = "|"))
)

# Frozen sensitivity analyses.
sensitivity_rows <- list()
run_sensitivity <- function(label, d, ratio_cut = 2.2, absolute_cut = 12, strict_outcome = FALSE,
                            extra_adjust = NULL, b12_cut = 200, mma_cut = 0.40, egfr_cut = 60) {
  d <- d[
    d$b12_pg_ml >= b12_cut & d$mma_umol_l <= mma_cut & d$egfr >= egfr_cut &
      d$alpha_tocopherol_umol_l >= absolute_cut,
    , drop = FALSE
  ]
  d$exposure2 <- ifelse(d$vit_e_chol_ratio < ratio_cut, "ratio_low", "adequate")
  required <- c("age", "sex", "race", "bmi", "smoking", "hba1c", "egfr", "b12_pg_ml", "log_mma", "folate_nmol_l", "pir", "albumin_g_dl")
  d <- d[complete.cases(d[, required, drop = FALSE]), , drop = FALSE]
  d$exposure2 <- factor(d$exposure2, levels = c("adequate", "ratio_low"))
  outcome <- if (strict_outcome) "pn_two_plus" else "pn_any"
  if (nrow(d) < 250 || sum(d$exposure2 == "ratio_low") < 25 || sum(d[[outcome]] == 1) < 20) return(NULL)
  rhs <- c("exposure2", "age", "sex", "race", "bmi", "smoking", "hba1c", "egfr", "b12_pg_ml", "log_mma", "folate_nmol_l", "pir", "albumin_g_dl", "cycle")
  if (!is.null(extra_adjust)) rhs <- c(rhs, extra_adjust)
  des <- make_design(d)
  f <- as.formula(paste(outcome, "~", paste(rhs, collapse = " + ")))
  m_pr <- svyglm(f, design = des, family = quasipoisson(link = "log"))
  m_rd <- svyglm(f, design = des, family = gaussian())
  zpr <- safe_effect(m_pr, "exposure2ratio_low", exponentiate = TRUE)
  zrd <- safe_effect(m_rd, "exposure2ratio_low")
  sensitivity_rows[[length(sensitivity_rows) + 1]] <<- data.frame(
    label = label, outcome = outcome, n = nrow(d), key_group_n = sum(d$exposure2 == "ratio_low"),
    key_group_events = sum(d[[outcome]][d$exposure2 == "ratio_low"]),
...
run_sensitivity("primary_absolute_normal_subset", dat)
run_sensitivity("strict_neuropathy_two_plus_sites", dat, strict_outcome = TRUE)
run_sensitivity("ratio_cut_2_0", dat, ratio_cut = 2.0)
run_sensitivity("ratio_cut_2_5", dat, ratio_cut = 2.5)
run_sensitivity("absolute_cut_11_6", dat, absolute_cut = 11.6)
run_sensitivity("strict_b12_mma", dat, b12_cut = 300, mma_cut = 0.30)
...
run_sensitivity("adjust_absolute_alpha_tocopherol", dat, extra_adjust = "log_alpha_tocopherol")
run_sensitivity("adjust_total_cholesterol", dat, extra_adjust = "total_cholesterol_mmol_l")
...
write.csv(model_df, file.path(out_dir, "c_n10_models.csv"), row.names = FALSE)
write.csv(sensitivity_df, file.path(out_dir, "c_n10_sensitivities.csv"), row.names = FALSE)
write.csv(nonlinear_df, file.path(out_dir, "c_n10_nonlinearity.csv"), row.names = FALSE)
write.csv(missingness_df, file.path(out_dir, "c_n10_missingness_mechanism.csv"), row.names = FALSE)

prep <- fromJSON(file.path(out_dir, "c_n10_prep_status.json"))
pr <- primary$pr
rd <- primary$rd
...
key_group_gate <- prep$key_group_n >= 250 && prep$key_group_events >= 40 && prep$validation_key_group_n >= 100 && prep$validation_key_group_events >= 15
clinical_gate <- is.finite(pr[1]) && is.finite(rd[1]) && pr[1] >= 1.35 && pr[2] >= 1.05 && rd[1] >= 0.03 && rd[2] >= 0.005
temporal_gate <- all(is.finite(c(dpr[1], vpr[1], drd[1], vrd[1]))) && dpr[1] > 1 && vpr[1] >= 1.20 && drd[1] > 0 && vrd[1] > 0
...
write_json(status, file.path(out_dir, "c_n10_status.json"), pretty = TRUE, auto_unbox = TRUE, na = "null")

report <- c(
  "# Blinded nutrition candidate C-N10: actual data gate report",
  "",
...
writeLines(report, file.path(out_dir, "c_n10_report.md"))
